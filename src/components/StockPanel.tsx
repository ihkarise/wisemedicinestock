    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={m.label} className="bg-[#18181b] border border-[#27272a] rounded-[16px] p-4 flex flex-col gap-1">
      ...
      {error && (
        <div className="bg-[#7f1d1d]/30 border border-[#ef4444]/40 rounded-[12px] px-4 py-3 flex items-start gap-3">
          ...
          <p className="text-[13px] text-[#fca5a5]"><span className="font-bold">Sync failed:</span> {error}</p>
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-2">
          {(["all", "low", "empty", "mt", "dil"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider transition-colors ${filter === f ? "bg-[#3b82f6] text-white" : "bg-[#27272a] text-[#a1a1aa] hover:text-[#fafafa]"}`}>
              {f === "all" ? `All (${total})` : f === "low" ? `Low (${low})` : f === "empty" ? `Empty (${empty})` : f.toUpperCase()}
            </button>
          ))}
          {inOrder > 0 && <span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-[#1d4ed8]/20 text-[#93c5fd] border border-[#1d4ed8]/30">{inOrder} in order</span>}
        </div>
      </div>
      ...
      {filteredMedicines.length === 0 ? (
        <div className="bg-[#18181b] border border-[#27272a] rounded-[20px] p-12 text-center">
          ...
          <p className="text-[#71717a] text-[14px]">{search ? `No medicines matching "${search}"` : "No medicines in this category."}</p>
        </div>
      ) : (
